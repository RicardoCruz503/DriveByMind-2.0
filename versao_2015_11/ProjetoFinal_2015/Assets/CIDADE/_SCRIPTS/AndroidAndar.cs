			...
			for (int i=0; i<Input.touchCount; i++) {
				if(Acelerar.GetComponent<GUITexture>().HitTest(Input.GetTouch(i).position)){
					if (Input.GetTouch (i).phase == TouchPhase.Began) {
						carregou = true;
						Helicopterozico.GetComponent<Rigidbody>().useGravity = false; 
					}if (Input.GetTouch (i).phase == TouchPhase.Ended) {
						carregou = false;
						Helicopterozico.GetComponent<Rigidbody>().useGravity=true;
					}
				}
				if(SetaCima.GetComponent<GUITexture>().HitTest(Input.GetTouch(i).position)){
						Helicopterozico.GetComponent<Rigidbody>().useGravity = false;
						texto.text = "Start";
						Helicopterozico.transform.Translate (0, velocidadeSubir*Time.deltaTime, 0);
				}
				if(SetaBaixo.GetComponent<GUITexture>().HitTest(Input.GetTouch(i).position)){
					Helicopterozico.transform.Translate (0, -(velocidadeSubir*Time.deltaTime),0);
			}
		}
		if (carregou == true) {
