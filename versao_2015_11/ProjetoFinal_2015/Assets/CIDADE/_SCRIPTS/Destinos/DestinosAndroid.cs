				...
				if (EscolhaDestino.activeSelf == true) {
						...
						if (Input.touches.Length <= 0) {
								...
								for (int i=0; i<Input.touchCount; i++) {
										if (LetraM.GetComponent<GUITexture>().HitTest (Input.GetTouch (i).position)) {
										...
										if (LetraZ.GetComponent<GUITexture>().HitTest (Input.GetTouch (i).position)) {
												if (Input.GetTouch (i).phase == TouchPhase.Began) {
														VirarEsquerda ();
														this.gameObject.GetComponent<NavMeshAgent> ().enabled = true;
														this.gameObject.GetComponent<NavMeshAgent> ().speed = Velocidade;
										...
										if (LetraY.GetComponent<GUITexture>().HitTest (Input.GetTouch (i).position)) {
												if (Input.GetTouch (i).phase == TouchPhase.Began) {
														SeguirEmFrente ();
														this.gameObject.GetComponent<NavMeshAgent> ().enabled = true;
														this.gameObject.GetComponent<NavMeshAgent> ().speed = Velocidade;
