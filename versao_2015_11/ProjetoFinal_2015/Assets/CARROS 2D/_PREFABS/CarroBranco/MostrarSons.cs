...
public class MostrarSons : MonoBehaviour {
	...
	void Update () {
		GameObject.Find("Pontuacaozica").GetComponent<GUIText>().text = "Pontuacao: " + pontuacao;
		if (Tempo > 1) { 
		...
		GameObject.Find("TempoJogo").GetComponent<GUIText>().text = "Tempo de Jogo: " + Mathf.Floor(Tempo) + " seg.";
	...
	void PlaySound(int numberClip){
		GetComponent<AudioSource>().clip = audioclip [numberClip];
